...
echo "==> 初始化：生成 outline/style/constraints/rubric + 初始计划"
echo "==> 你应该先把第一版书稿放到 contents/ 或把分章稿放到 docs/"
...
claude -p "$(cat "$PROMPT_FILE")" > "$LOG"

echo "==> 初始化完成："
echo " - outline.md / style_guide.md / constraints.md / rubric.md"
echo " - $PLAN"
echo " - 日志：$LOG"
echo " - Prompt：$PROMPT_FILE"

echo "==> 下一步：打开 $PLAN 看 7 天游标；然后跑 scripts/daily.sh 执行第 1 天游标。"